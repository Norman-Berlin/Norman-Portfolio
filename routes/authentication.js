...
router.post('/register', async (req, res) => {
    ...
    try {
      ...
      // Validate dob
      const validatedob = `SELECT * FROM micah_supporters WHERE dob = ?;`
      const [dobResults] = await pool.promise().execute(validatedob, [dob]);
    ...
    //   const hashedPassword = await bcrypt.hash(password, 10);
      ...
      // const insertQuery = INSERT INTO micah_supporters (username, email, dob, password, verification_token) VALUES (?, ?, ?, ?, ?);
      // await pool.promise().execute(insertQuery, [fullname, email, dob, hashedPassword, verificationToken]);
  
      const insertQuery = `INSERT INTO micah_supporters (username, email, dob, password) VALUES (?, ?, ?, ?);`
      await pool.promise().execute(insertQuery, [fullname, email, dob, password]);
      // Insert the user into the database only if email sending succeeds
